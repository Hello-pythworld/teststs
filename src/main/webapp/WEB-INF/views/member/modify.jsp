...
<body>
	<form action="mModify" method="post" enctype="multipart/form-data">
		<table>
			...
			<tr>
				<th>아이디</th>
				<td><input type="hidden" name="mId" value="${modify.MId}" />
					${modify.MId}</td>
			</tr>
			...
			<tr>
				<th>성별</th>
				<c:choose>
					<c:when test="${modify.MGender eq '남자'}">
						<td>
							남자 <input type="radio" name="mGender" value="남자" checked/> 
					...
					<c:otherwise>
						<td>
							...
							여자 <input type="radio" name="mGender" value="여자" checked/>
						</td>
					</c:otherwise>
			...
			<tr>
				...
				<td>
					...
					<img id="preview" width="150px" src="./resources/profile/${modify.MProfileName}"/>
					<input type="hidden" name="mProfileName" value="${modify.MProfileName}" />
				</td>
			</tr>
